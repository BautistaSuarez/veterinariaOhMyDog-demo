...
def ver_turnos (request):
    if request.method == 'POST' :
        ...
        if not fecha_fin or not fecha_inicio:
            ...
            return redirect('turnos:mostrar_turnos')
        turnos = Turno.objects.filter(Fecha__range=[fecha_inicio, fecha_fin], Pendiente=True).order_by('Fecha')
        ...
        turnos = Turno.objects.exclude(Pendiente=False).order_by('Fecha')
        paginator = Paginator(turnos, 20)
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
        mostrar = turnos.count() > 0 and paginator.num_pages > 1
...
def rechazar_turno(request, id):
    ...
    message = f"El turno del dia {turno.Fecha} fue cancelado, por favor solicite otro turno"
...
def mis_turnos(request):
    usuario = get_object_or_404(CustomUser, id=request.user.id)
    turnos = Turno.objects.filter(id_usuario_id=request.user.id)
